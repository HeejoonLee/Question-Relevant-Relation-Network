...
from model.regat_all import build_regat_all
from config.parser import parse_with_config
from train_all import train
from train_all import evaluate
import utils
...
if __name__ == '__main__':
    ...
    # 1. Set device and batch size
    print("[LOG] 1. Setting device and batch size...")
    ...
    # 2. Set seed and fusion method
    print("[LOG] 2. Setting seed and fusion method...")
    if args.seed != -1:
    ...
    # 3. Load word vectors
    print("[LOG] 3. Loading word vectors...")
    dictionary = Dictionary.load_from_file(
                    join(args.data_folder, 'glove/dictionary.pkl'))


    # 4. Load dataset
    print("[LOG] 4. Loading training, validation datatset...")
    ...
    # 8. Load dataloader
    print("[LOG] 8. Loading train, eval loader...")
    train_loader = DataLoader(train_dset, batch_size, shuffle=True,
                              num_workers=4, collate_fn=trim_collate, drop_last=True)
    eval_loader = DataLoader(val_dset, batch_size, shuffle=False,
                             num_workers=4, collate_fn=trim_collate, drop_last=True)

    output_meta_folder = join(args.output, "regat_all")
